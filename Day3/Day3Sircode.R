...
data <- tibble(ID, Sex, Age, Score);data
data<-data.frame(ID, Sex, Age, Score);data

data1 = as.data.frame(data);data1
...
#row_number()
# Using row_number() with mutate() will create a column of consecutive numbers. The row_number() function is useful for creating an identification number (an ID variable). It is also useful for labeling each
